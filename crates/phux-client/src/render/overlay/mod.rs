...
fn emit_buffer(out: &mut impl Write, buf: &Buffer) -> io::Result<()> {
    ...
    for row in 0..area.height {
        ...
        for col in 0..area.width {
            ...
            emit_cell_sgr(out, cell, &mut prev_styled)?;
            let sym = cell.symbol();
            if sym.is_empty() {
                out.write_all(b" ")?;
            } else {
...
/// Emit SGR for one cell.
///
/// Skips emission entirely when the cell has no styling (default fg/bg,
/// no modifier bits) AND the previous cell also had none, so a long
/// run of unstyled text emits no SGR sequences at all. When *any*
/// styling is present, emits a full reset + the cell's attributes so
/// stale style from a previous cell can't leak.
///
/// `prev_styled` is updated by the caller after each cell — passing
/// `&mut bool` instead of a [`ratatui::buffer::Cell`] keeps the
/// trailing-state check trivially correct.
fn emit_cell_sgr(
    out: &mut impl Write,
...
/// for `Color::Reset` (no override).
///
/// Indexed ANSI colors are mapped to a small fixed palette so the
/// overlay renders consistently regardless of the user's terminal
/// theme. The chrome submodule may later grow theme awareness; the
/// overlay stays self-contained for v1.
