...
not interpret. Scopes are Terminal, Collection, and Global; values are
...
- `Collection { collection_id, key, value }` — `CollectionId` is an opaque
...
Scope = tagged_union {
    ...
    COLLECTION (CollectionId),   // tag 0x01; u32 wire body
...
MetadataScope = tagged_union {
    TERMINAL   (TerminalId),
    COLLECTION (CollectionId),   // opaque grouping key, not a tier
    GLOBAL,
}
```
...
- **`phux.session.name/v1`** — the human-facing group name. A `SET` on this
  ...
  string. Scope: the group's `CollectionId`.
...
`CollectionId` through these keys. The server stores it without interpreting,
...
Scoped to a `CollectionId`. The binary-split layout tree the reference TUI
...
- **`phux.tui.window_order/v1`** — scoped to a `CollectionId`; a `list<u32>`
