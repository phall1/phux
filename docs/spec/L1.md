...
Command_L1 = tagged_union {
    ...
    GET_SCREEN       { terminal_id: TerminalId, request_scrollback: optional<u32> },
...
Unlike `ATTACH_TERMINAL`, it neither subscribes the caller nor resizes
the Terminal, so it is safe to poll against a pane other clients are
using. It is the read floor of the agent surface
([ADR-0022](../../ADR/0022-tool-for-agents.md)). Allowed for viewers.
The richer per-cell projection (semantic marks, styles) is a future
additive field on the same `JSON` reply, not a new command.
...
viewport `lines[]` are unchanged. `ScreenState.schema_version` is `2`
when this field is part of the contract; `scrollback[]` is serde-default,
so a `schema_version = 1` consumer reading a v2 reply ignores the extra
key. Walking history is still side-effect-free — the server reads history
cells in place and does not scroll the live viewport (`phux-o1v`).
