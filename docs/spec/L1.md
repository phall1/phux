...
Command_L1 = tagged_union {
    ...
    GET_SCREEN       { terminal_id: TerminalId },
...
(`{ schema_version, pane, cols, rows, cursor?, lines[] }`). Unlike
`ATTACH_TERMINAL`, it neither subscribes the caller nor resizes the
Terminal, so it is safe to poll against a pane other clients are using.
It is the read floor of the agent surface
