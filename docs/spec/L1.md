...
| 0x06 | `dirty`            | (empty)                    |
| 0x07 | `idle`             | (empty)                    |
| 0x08 | `asked`            | field-tagged TLV (below)   |

Unlike the other bodies (positional), the `asked` (0x08) body is itself
...
- `asked` — sourced when an agent in the scoped Terminal blocks on a
  human-answerable question. The control-plane carrier for a pending prompt: a
  projection consumer renders the waiting state (id, text, suggested answers,
  how long it has waited) from the event rather than re-deriving it from the
  grid. `suggestions` are the *actual options* (not yes/no), in presentation
  order; `elapsed_seconds` is the wait duration when reported.
