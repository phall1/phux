...
| 0x41  | C → S     | `SUBSCRIBE_EVENTS`   | §7.5               | partial   |
...
`TERMINAL_CLOSED` is the single L1 lifecycle event for a Terminal
ceasing to exist, whether it died from PTY EOF / process exit, a
`KILL_TERMINAL` command, or a `KILL_COLLECTION` teardown. The server
MUST emit it to every client subscribed to the Terminal and MUST NOT
additionally infer a client `DETACHED` from a Terminal's death
(ADR-0015 L1: the server reports lifecycle *facts*; it does not
interpret them). Whether the death of a Terminal should detach a
client is a **consumer** policy, not a server one. The reference TUI
folds the closed Terminal out of its layout on `TERMINAL_CLOSED` and
detaches itself only once no Terminals remain in its attached
collection. A client still detaches explicitly via `DETACH` (proto.md
§7.3) and the server still sends `DETACHED` for server-initiated
teardown (e.g. shutdown, takeover); EOF is no longer one of those
cases.
...
Command_L1 = tagged_union {
    ...
    RENAME_SESSION   { collection: CollectionId, name: str, new_name: str },
}
...
(`{ schema_version, pane, cols, rows, cursor?, lines[], scrollback[],
cells? }`). Unlike `ATTACH_TERMINAL`, it neither subscribes the caller
...
`{ col, row, semantic?, style: { bold, faint, italic, underline, blink,
inverse, invisible, strikethrough, overline, fg, bg } }`. `semantic` is
...
[input.md](./input.md)) to `terminal_id` without an `ATTACH_TERMINAL`,
...
its dimensions. The reply is `COMMAND_RESULT { OK }`,
`ERROR { TERMINAL_NOT_FOUND }` for an unknown id, or
`ERROR { PERMISSION_DENIED }` when the caller is not `PRIMARY` for the
target Terminal ([input.md §7](./input.md)). Input is
fire-and-forget ([input.md §7](./input.md)): if the Terminal's input
...
([ADR-0021 §3](../../ADR/0021-control-plane-commands.md)). Allowed for
primaries.
...
`RENAME_SESSION` is the rename counterpart to `CREATE_SESSION`: it
resolves the session named `name` under `collection` (the same name
resolution `KILL_COLLECTION` uses) and reassigns its human-readable name
to `new_name` in *one* round-trip. The reply is `COMMAND_RESULT { OK }`.
The server is authoritative: it owns the session→name mapping, so the
rename takes effect server-side immediately and each attached client
reconciles the new name on its next `ATTACHED` snapshot. An unknown `name`
is refused with `ERROR { SESSION_NOT_FOUND }`; a `new_name` already in use
is refused with `ERROR { INVALID_COMMAND }` (the same rule `CREATE_SESSION`
applies to a taken name — names are unique within a `collection`). v0.1
servers host only the default `CollectionId(1)`. A live `SESSION_RENAMED`
push to other attached clients (the L2-foreshadowed event) is not yet
wired; snapshot reconciliation covers the rename until it lands. It backs
`phux rename SESSION NEW-NAME` and the TUI `rename-session` action
([ADR-0021 §3](../../ADR/0021-control-plane-commands.md)). Allowed for
primaries.
...
## 7.5 Agent event stream

The push half of the agent surface (phux-y2t,
[ADR-0022](../../ADR/0022-tool-for-agents.md) 'events' phase). A client
SUBSCRIBES to a stream of extensible tagged lifecycle / activity events,
and the server PUSHES `EVENT` frames as those events occur. This is an
**additive accelerator** of the CLI-side poll-floor `wait` (which ships
over `GET_SCREEN`, §5): match conditions stay evaluated client-side, but
an event wakes the waiter immediately instead of on the next poll tick,
cutting latency without changing correctness. A consumer that ignores the
stream entirely still converges via polling.

Wire bodies (positional, per
[appendix-encoding.md](./appendix-encoding.md) primitives):

```
SUBSCRIBE_EVENTS { terminal: optional<TerminalId> }
EVENT            { terminal: optional<TerminalId>, event: AgentEvent }
```

`SUBSCRIBE_EVENTS.terminal` scopes the subscription: `Some(id)` delivers
only that Terminal's events (per-pane); `None` delivers every event the
server emits for any Terminal the client may observe (server-scoped) —
including `pane_spawned` / `pane_closed` across the attached collection.
Subscription is idempotent (re-subscribing the same scope is a no-op) and
is implicitly torn down on detach, matching `SUBSCRIBE_METADATA` (an
explicit `UNSUBSCRIBE_EVENTS` is a future ticket). Subscribing does NOT
attach, resize, or send a snapshot — it is a pure push registration, so
an agent can `watch` a Terminal without disturbing the live session.

`EVENT.terminal` identifies the Terminal the event concerns, or `None`
for a server-scoped event with no single owning Terminal.

### 7.5.1 AgentEvent taxonomy

`AgentEvent` is an extensible tagged union. It is encoded **TLV**: a
`tag: u8` followed by a length-prefixed `body: bytes`. The length prefix
is the forward-compatibility lever — a decoder that does not recognise
`tag` reads (and skips) the declared body length and surfaces the event
as an opaque `Unknown { tag, body }`, so a later minor version MAY add
event kinds and an older client skips them cleanly rather than failing
the frame parse. Tags are allocated sequentially:
...
Event sourcing (reference server, phux-y2t):

- `pane_spawned` / `pane_closed` — sourced from the existing Terminal
  lifecycle (`SPAWN_TERMINAL` and the PTY-EOF / `KILL_TERMINAL` path that
  already emits `TERMINAL_CLOSED`, §1.1). `pane_closed.exit_status`
  carries the same value as `TERMINAL_CLOSED.exit_status`.
- `title_changed` — sourced from libghostty's OSC 0 / OSC 2 title
  tracking; the server polls the Terminal title after each PTY chunk and
  emits when it changes.
- `bell` — sourced from a BEL (`0x07`) in the PTY byte stream; the
  control-plane counterpart to the `BELL` frame (`0xB0`).
- `dirty` / `idle` — sourced from the per-pane state-sync tick's dirty
  flag. The server coalesces: at most one `dirty` per active output
  burst, then one `idle` when the grid has settled across an idle window.

- `command_started` / `command_finished` — sourced from OSC-133 semantic
  prompt marks. **Exit-code gap:** `command_started` is emitted from the
  `B`/`C` (command-start) marks the grid exposes today; `command_finished`
  is emitted from the `D` (command-end) mark. The shell-integration exit
  code (`OSC 133 ; D ; n ST`) is NOT reliably extractable from the grid
  cell-semantic projection the reference server reads, so
  `command_finished.exit_code` is `None` until that plumbing lands. The
  wire field is allocated (`optional<i32>`) so a future server can fill
  it without a wire change. See the changelog entry for the precise
  deferral.
