...
- **Scope.** Aggregated `GET_STATE`/LIST across satellites, satellite-targeted
  `SPAWN_TERMINAL` addressing, and full two-hop `ATTACH` semantics
  (predictive echo, snapshot invariants) are defined separately; until the
  attach relay lands, attach-scoped frames relayed to a satellite are subject
  to the satellite's own attach/subscription gates. `ROUTE_INPUT` is the
  attach-free input path through a hub.
