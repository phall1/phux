...
- **Input-lease aliasing (phux-v45.7).** Every hub consumer reaches a
  ...
  (ADR-0038).
- **Scope.** Aggregated `GET_STATE`/LIST across satellites, satellite-targeted
  `SPAWN_TERMINAL` addressing, and predictive-echo tuning across the extra
  hop are defined separately. `ROUTE_INPUT` remains the attach-free input
  path through a hub (lease-gated as above).
