...
Encoding is positional (big-endian, length-prefixed), per
[appendix-encoding.md](./appendix-encoding.md), which owns the normative codec
statement. The wire bodies below are written in that positional shape.
...
`collection` is `CollectionId`, a documented opaque grouping key, not a
lifecycle tier. The reference server exposes a single default value at
`CollectionId(1)`; other ids MAY surface as `SpawnError::CollectionNotFound`.
`CollectionId` is retained because some `SPAWN_TERMINAL` and L3 scoping paths
still carry it; its full removal is tracked as future work (bead phux-0bmc). It
SHALL NOT be read as a collection lifecycle tier — there is none.
...
CommandValue = tagged_union {
    TERMINAL_ID(TerminalId),
    COLLECTION_ID(CollectionId),  // opaque grouping key, not a tier
    STATE(StateSnapshot),
...
Command_L1 = tagged_union {
    SPAWN            { cwd: optional<str>, command: optional<list<str>>,
                       ...
                       collection: optional<CollectionId> },        // opaque grouping key
...
(positional, per [appendix-encoding.md](./appendix-encoding.md)).
...
AttachTarget = tagged_union {
    ...
    BY_COLLECTION_ID(CollectionId),  // opaque grouping key
...
When multiple clients attach to the same Terminal with different viewport sizes,
the server uses the smallest common bounding box (configurable: `aggressive`
mode resizes per attached client). This matches tmux's behavior and avoids
surprising shrink-and-grow on attach/detach. How Terminals are laid out within
an attached client's viewport is a consumer concern: TUIs paint borders and
chrome; agents may not paint anything; layout-tree state is L3 metadata (see
[L3.md](./L3.md)), not a wire concept.
