...
last-focused session; `send-keys`, `run`, `ask`, and every spatial verb require
it. Spatial verbs are stricter than the selected-pane tiebreak: each selector
must match exactly one local pane.
