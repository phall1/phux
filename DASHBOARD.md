...
Generated 2026-08-03T06:02:26Z by the ci-metrics workflow. Do not edit —
...
|---|---:|---:|---:|---:|
| test | 430 | 2s | 10m43s | 17m30s |
| check | 431 | 2s | 2m33s | 5m11s |
| detect docs-only | 434 | 2s | 5s | 8s |
...
| check | rust checks (fmt + clippy + doc + deny) | 3m39s | 14 |
...
|---|---|---:|---:|
| ci / check | clippy | 1m39s | 240 |
| ci / check | doc | 10s | 240 |
| ci / check | deny | 3s | 238 |
| ci / check | fmt | 2s | 247 |
| ci / test | unit | 12m02s | 219 |
| ci / test | e2e | 12s | 214 |
...
## Slowest tests (latest instrumented run, `dbcc95b06`)

| test | wall |
|---|---:|
| `phux-server::spawn_terminal::spawn_terminal_default_term_is_xterm_256color` | 15.080s |
| `phux-server::runtime::attach::tests::prepare_attach_rejects_pane_source_count_before_registration` | 1.655s |
| `phux-server::terminal_actor::tests::xtwinops_size_queries_answered_from_resized_geometry` | 1.448s |
...
| `phux-server::server_idle_exit::connecting_disarms_the_idle_clock` | 1.066s |
