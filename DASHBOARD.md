...
Generated 2026-08-03T07:22:06Z by the ci-metrics workflow. Do not edit —
...
|---|---:|---:|---:|---:|
| test | 434 | 2s | 10m40s | 17m34s |
| check | 435 | 2s | 2m35s | 5m15s |
| detect docs-only | 438 | 2s | 5s | 8s |
...
| check | rust checks (fmt + clippy + doc + deny) | 3m55s | 16 |
| check | Run Swatinem/rust-cache@v2 | 15s | 18 |
...
|---|---|---:|---:|
| ci / check | clippy | 1m42s | 244 |
| ci / check | doc | 10s | 244 |
| ci / check | deny | 3s | 242 |
| ci / check | fmt | 2s | 251 |
| ci / test | unit | 11m56s | 223 |
| ci / test | e2e | 12s | 216 |
...
## Slowest tests (latest instrumented run, `c8badafae`)

| test | wall |
|---|---:|
| `phux-server::runtime::attach::tests::prepare_attach_rejects_pane_source_count_before_registration` | 1.625s |
