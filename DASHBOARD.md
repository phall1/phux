...
Generated 2026-08-02T00:48:47Z by the ci-metrics workflow. Do not edit —
...
|---|---:|---:|---:|---:|
| test | 266 | 2s | 13m26s | 17m49s |
| check | 264 | 2s | 3m16s | 5m12s |
| detect docs-only | 269 | 2s | 5s | 8s |
...
| check | rust checks (fmt + clippy + doc + deny) | 2m43s | 19 |
| check | runner disk headroom | 38s | 19 |
| test | runner disk headroom | 31s | 19 |
...
|---|---|---:|---:|
| ci / check | clippy | 1m55s | 147 |
| ci / check | doc | 11s | 147 |
| ci / check | deny | 4s | 146 |
| ci / check | fmt | 2s | 150 |
| ci / test | unit | 13m22s | 134 |
| ci / test | e2e | 10s | 131 |
| ci / test | agents-smoke | 1s | 72 |
...
## Slowest tests (latest instrumented run, `c9d20484b`)

| test | wall |
|---|---:|
| `phux-server::perf_bursty_output::synthesize_against_reference_alloc_bounded_under_full_churn` | 106.865s |
| `phux-server::terminal_actor::tests::resize_desync_then_both_shrink_does_not_overflow` | 27.782s |
...
| `phux-record::golden_cast::both_containers_agree_on_the_frame_count_for_one_recording` | 3.808s |
